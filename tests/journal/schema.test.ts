...
import decisionSchema from "../../spec/journal/1.0.0/signed-decision.schema.json" with {type: "json"};
import {createIdentity} from "../../packages/identity/src/index.js";
import {createJournalReview, createJournalVersion, signJournalDecision, signJournalReview} from "../../packages/journal/src/index.js";
...
describe("期刊出版协议 Schema", () => {
  ...
  it("严格校验签名评审与人类编辑责任字段", async () => {
    const reviewer = await createIdentity(); const editor = await createIdentity(); const paperId = `sha256:${"a".repeat(64)}`; const versionId = `sha256:${"b".repeat(64)}`;
    ...
    const decision = signJournalDecision({paper_id: paperId, version_id: versionId, editor_id: editor.agentId, decision: "accept", rationale: "达到出版标准", review_ids: [review.review_id, `sha256:${"c".repeat(64)}`], decided_at: "2026-09-03T11:00:00.000Z"}, editor);
    const ajv = new Ajv2020({strict: true, strictRequired: false, validateFormats: false});
    expect(ajv.validate(reviewSchema, review), ajv.errorsText()).toBe(true); expect(ajv.validate(decisionSchema, decision), ajv.errorsText()).toBe(true);
