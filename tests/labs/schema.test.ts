...
import {WORLD_SUPPLY_SCHEDULE_BODY, WORLD_SUPPLY_SCHEDULE_ID, createWorldSupplyState} from "../../packages/kernel/src/index.js";
...
describe("LABS JSON Schemas", () => {
  ...
  it("accepts reference objects and rejects malformed or expanded objects", async () => {
    ...
    const branch = createLabsWorldBranch(REFERENCE_RULESET, {world_fork_id: REFERENCE_FORK_ID, region_id: "public", resource_id: "resource-alpha", schedule_id: WORLD_SUPPLY_SCHEDULE_ID, research_height: 0, subsidy: 8, previous_settlement_id: WORLD_SUPPLY_SCHEDULE_ID, length: 451});
    ...
    expect(validateWorldBranch({...branch, research_height: -1})).toBe(false);
    ...
    expect(validateSupplySchedule({...WORLD_SUPPLY_SCHEDULE_BODY, allocations: [WORLD_SUPPLY_SCHEDULE_BODY.allocations[0], WORLD_SUPPLY_SCHEDULE_BODY.allocations[0], WORLD_SUPPLY_SCHEDULE_BODY.allocations[2]]})).toBe(false);
    expect(validateSupplySchedule({...WORLD_SUPPLY_SCHEDULE_BODY, ruleset_id: `sha256:${"0".repeat(64)}`})).toBe(false);
    expect(validateSupplyState({...createWorldSupplyState(), research_height: 8_401})).toBe(false);
