...
export function createLabsAwareApplication(world: SaiRegionApplication, labs: LabsRepository): SaiRegionApplication {
  return {
    ...
    async act(agentId: string, input: ActInput): Promise<ActResult> {
      ...
      try {
        if (!args.result || !args.result_id || !args.signed_claim || !args.claim_id || !args.research_task || !args.task_id || !args.method_artifact || !args.artifact_id || !args.research_record || !args.record_id || !/^network:sha256:[0-9a-f]{64}$/.test(args.economic_network_id ?? "")) throw new TypeError("LABS 世界研究动作必须由桥接器生成完整可复现记录并签名");
