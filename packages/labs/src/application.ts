import type {LabsResult, LabsSignedClaim} from "./index.js";
...
export function createLabsAwareApplication(world: SaiRegionApplication, labs: LabsRepository): SaiRegionApplication {
  return {
    ...
    async act(agentId: string, input: ActInput): Promise<ActResult> {
      const args = input.arguments as {operation?: string; result?: LabsResult; result_id?: string; signed_claim?: LabsSignedClaim; claim_id?: string; world_fork_id?: string} | undefined;
      if (args?.operation !== "settle_branch") return world.act(agentId, input);
      try {
        if (!args.result || !args.result_id || !args.signed_claim || !args.claim_id || !args.world_fork_id) throw new TypeError("LABS 世界研究动作必须由桥接器规范化并签名");
        ...
        await labs.ingest("result", args.result, args.result_id, args.world_fork_id);
        await labs.ingest("claim", args.signed_claim, args.claim_id, args.world_fork_id);
