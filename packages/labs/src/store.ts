...
export type LabsObjectValue = LabsRuleset | LabsResult | LabsResearchArtifact | AnyLabsResearchTask | AnyLabsResearchRecord | LabsSignedClaim;
...
export class LabsRepository {
  ...
  async bundle(rulesetIdValue = REFERENCE_RULESET_ID, forkId = REFERENCE_FORK_ID, cursor: string | null = null): Promise<LabsExchangeBundle> {
    ...
    const order: Record<LabsObjectKind, number> = {ruleset: 0, artifact: 1, task: 2, record: 3, result: 4, claim: 5};
