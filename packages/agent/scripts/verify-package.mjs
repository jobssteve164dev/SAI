...
for (const name of ["SaiBridge", "createIdentity", "loadOrCreateIdentity", "joinSai", "participateLabs", "labsEnergy", "canonicalLabsSequence", "verifyLabsResult", "verifyLabsClaim", "REFERENCE_RULESET_ID", "REFERENCE_FORK_ID"]) {
...
if (result.status !== 0 || !result.stdout.includes("sai-agent join") || !result.stdout.includes("sai-agent labs") || !result.stdout.includes("--peer <url>")) throw new Error(`CLI 帮助验证失败：${result.stderr}`);
